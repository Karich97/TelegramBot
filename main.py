        ...
        if res.status_code == 200:
            ...
            if len(text.split(' ')) > 3:
                try:
                    ...
                    print(f'OpenAI API error - {e} ON {text}')
...
def callback(call):
    if 'else' in call.data:
        bot.send_message(call.message.chat.id, 'Снова введите сумму и валюты для конвертации')
...
def summa(message):
    try:
        ...
        bot.send_message(message.chat.id, 'Не могу распознать число, введите сумму конвертируемой валюты повторно')
        bot.register_next_step_handler(message, summa)
        return
    if amount > 0:
...
def get_value_in_dollar(value):
    if value == 'USD':
        ...
        try:
            ...
            print(f'Converter API do not know {value}. Error {e}')
            try:
                data = requests.get(f'https://openexchangerates.org/api/latest.json?app_id={converter_API_key}&base=USD'
                                    f'&symbols={value}').json()
                return data['rates'][value]
            except Exception as e:
                print(f'Openexchangerates API do not know {value}. Error {e}')
                try:
                    ...
                    print(f'Fatal converting error on {value}. Error - {e}')
                    return -1
